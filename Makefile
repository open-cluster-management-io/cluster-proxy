...
test-integration: manifests generate fmt vet
	...
	source ${ENVTEST_ASSETS_DIR}/setup-envtest.sh; \
		...
		go test ./test/integration/... -coverpkg=./pkg/... -coverprofile $(INTERGRATRION_TEST_COVERAGE_FILE)
...
build-e2e:
	go test -c --cover --coverpkg=./pkg/... -o bin/e2e ./test/e2e/

test-e2e: build-e2e
	./bin/e2e -test.coverprofile $(E2E_TEST_COVERAGE_FILE) --test-cluster $(E2E_TEST_CLUSTER_NAME)
